...
class BaseService(ABC):
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to start service '{self.name}': {e}", exc_info=True
            )
            return False
        ...
        try:
            ...
            if self._task:
                try:
                    ...
                    self.logger.warning(
                        f"Service '{self.name}' did not stop gracefully, cancelling"
                    )
                    self._task.cancel()
                    try:
                        await self._task
            ...
            self.status = ServiceStatus.ERROR
            self.logger.error(
                f"Failed to stop service '{self.name}': {e}", exc_info=True
            )
            return False
        ...
        try:
            while not self._stop_event.is_set():
                try:
                    ...
                    self.logger.error(
                        f"Error in service '{self.name}' execution: {e}", exc_info=True
                    )
                    # Call error handler
            ...
            self.status = ServiceStatus.ERROR
            self.logger.error(
                f"Fatal error in service '{self.name}': {e}", exc_info=True
            )
